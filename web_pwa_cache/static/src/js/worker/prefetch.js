...
PWA.include({
    ...
    _prefetchModelData: function () {
        return new Promise(async (resolve) => {
            ...
            for (const index in prefetched_models) {
                ...
                // Get current records
                try {
                    const cur_model_record = await this._importer._getModelRecords(
                        model_def.model
                    );
                    if (cur_model_record.prefetch_last_update) {
                        domain_forced = [
                            ["write_date", ">=", cur_model_record.prefetch_last_update],
                        ];
                    }
                } catch (err) {
                    // do nothing.
    ...
    _prefetchActionData: function () {
        return new Promise(async (resolve) => {
            ...
            let [response, request_data] = await this._rpc.sendJSonRpc(
                "/pwa/prefetch/action",
                {
     ...
     * @returns {Promise}
     */
    _prefetchFunctionData: function () {
        return new Promise(async (resolve) => {
            const prefetch_last_update = await this._config.get(
                "prefetch_onchange_last_update"
            );
            this.postClientPageMessage({
                type: "PREFETCH_MODAL_TASK_INFO",
                id: "function_data",
                message: "Getting function calls data...",
            });
            // Get prefetching metadata
            let [response, _] = await this._rpc.sendJSonRpc("/pwa/prefetch/function");
            const response_data = (await response.json()).result;
            if (response_data) {
                this._importer.saveFunctions(response_data);
            }
     ...
     * @returns {Promise}
     */
    _prefetchClientQWebData: function () {
        return new Promise(async (resolve) => {
            const prefetch_last_update = await this._config.get(
                "prefetch_view_last_update"
            );
            this.postClientPageMessage({
                type: "PREFETCH_MODAL_TASK_INFO",
                id: "clientqweb_data",
            ...
            // Get prefetching metadata
            let [response] = await this._rpc.sendJSonRpc("/pwa/prefetch/clientqweb", {
                last_update: prefetch_last_update,
            });
            // Prefetch Templates
            const response_data = (await response.json()).result;
            if (response_data) {
                ...
                for (const xml_ref in response_data) {
                    this.postClientPageMessage({
                        type: "PREFETCH_MODAL_TASK_INFO",
                        id: "clientqweb_data",
                        message: "Getting templte " + xml_ref + "...",
                        progress: index / num_views,
                    });
                    const [
                        response,
