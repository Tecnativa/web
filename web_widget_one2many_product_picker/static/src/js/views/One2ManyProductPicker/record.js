    ...
    function (require) {
        ...
        var One2ManyProductPickerRecord = Widget.extend({
            ...
            destroy: function () {
                this.abortTimeouts();
                if (this.state) {
                    this.options.basicFieldParams.model.removeVirtualRecord(
                        this.state.id
                    );
                }
                this.$el.remove();
            ...
            _getEvalContext: function () {
                ...
                var PY_floatFixed = new py.PY_def.fromJSON(
                    function () {
                        var args = py.PY_parseArgs(arguments, ["str"]);
                        return py.str.fromJSON(
                            this._getFloatFieldValue(args.str.toJSON())
                        );
                    }.bind(this)
                );
                return {
             ...
             * @returns {Deferred}
             */
            _generateVirtualStateDeferred: function (context, def_values) {
                var self = this;
                var model = this.options.basicFieldParams.model;
                var def = $.Deferred();
                    ...
                    .then(() => {
                        ...
                        return model
                            ...
                            .then(() => {
                                ...
                                return self.recreate(model.get(record_def.record.id));
                                // .then(function() {
            ...
            _generateVirtualState: function (data, context, options) {
                ...
                var def_values = {
                    ...
                    [this.options.fieldMap[this.master_uom_map.field_uom]]:
                        this.recordSearch[this.master_uom_map.search_field_uom][0],
                };
                return this._generateVirtualStateDeferred(
                    scontext,
                    def_values,
                    options
                );
            ...
            _applyChanges: function (record_id, changes, options) {
                ...
                var model = this.options.basicFieldParams.model;
                return model
                    ._applyChange(record_id, changes, options)
                    .then(function () {
            ...
            _updateRecord: function (amount) {
                ...
                if (amount === 0) {
                    ...
                    changes[this.options.fieldMap[this.master_uom_map.field_uom_qty]] =
                        lazy_qty;
                } else if (
                    changes[
                        this.options.fieldMap[this.master_uom_map.field_uom_qty]
            ...
            _onClickFlipCard: function (evt) {
                ...
                if (
                    !this.options.readOnlyMode &&
                    this.getParent().canBeSaved(this.state.id)
                ) {
                    if (
